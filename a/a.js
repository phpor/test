...
function SSOController() {
	...
	this.loginTimeout = 5000;	// 登录超时时间 5000ms
	...
	this.getVersion = function() {
		return "ssologin.js(v1.4.9) 2013-04-09";
	};
	...
	this.getLoginInfo = function(){
		...
		if (!sso_info) return {};
		if (sso_info.substr(0,3) == "v01") {
			return parse_str(urldecode(sinaSSOEncoder.Cookie.decode(sso_info.replace(/^v01/,""))));
		}
		return parse_str(urldecode(getCookie('sso_info')));
	};
	...
	//此函数为用户可覆盖的回调函数，loginStatus参数为用户的登录状态对象，result属性为false则为未登录。
	//该方法由产品自己来定义
	...
	(function(){
		var $empty = function(){},
			//允许被传入的参数包括 loginParam 和 loginOptions 里面列举的属性。
			//loginParam 中列举的属性不会被混合到ssoController实例对象
			loginParam = {
				//必填。用户名。
				username : '',
				//必填。密码。
				...
				//选填。是否保留登录状态，单位为天。
				savestate : 0,
				//选填。是否支持动态码验证，0为不支持，1为支持。
				//开启后会验证用户是否有动态码，并验证动态码是否输入正确。
				vsnf : 0,
				//开启动态码验证后必填。用户的动态码字符串。
				vsnval : '',
				//选填。验证码的值，填写后才会进行验证，未填写默认不做此验证。
				door : '',
				//选填。是否在登录后在当前页面种新浪域 cookie。
				//如果设置为0，新浪域登陆验证可以通过，但页面仍然处于未登录新浪域的状态。
				setCookie : 0,
				//选填。登陆后会话cookie保持多长时间。
				cookieTimeout : 0,
				//选填。跨域登录时，可以用来减少一次登录请求。
				//必须设置feedBackUrl才能开启这个值。目前仅在微博域使用，其他域不使用这个值。
				ssoSimpleLogin : 0,
				onComplete : $empty,	//登录流程完成回调
				onSuccess : $empty,		//登录成功回调
				onFailure : $empty		//登录失败回调
			},
			logoutParam = {
				onComplete : $empty,	//退出登录完成回调
				onSuccess : $empty,		//退出登录成功回调
				onFailure : $empty		//退出登录失败回调
			},
			//下面列举的属性，将用于标记如何将 loginParam 对应的属性混合进 loginExtraQuery
			hashExtra = {
				vsnf : 'vsnf',
				vsnval : 'vsnval',
				door : 'door',
		...
		//自定义登录组件准备工作
		//param {Object} spec 登录选项
		customPrepare = function(spec){
			var key;
			spec = spec || {};

			//在这里列举可以被混合到ssoController对象实例的属性
			loginOptions = objMerge({
				// -------- 通常的设置项 start --------
				//必填。登录入口，用于登录后端记录日志，标识为哪个项目。
				entry : 'sso',
				// -------- 通常的设置项 end --------

				// -------- 用于跨域登录的配置项 start --------
				//跨主域必填。登录成功或失败后是否返回ticket（票据），默认为false，跨主域登录应设为true。
				useTicket : false,
				//跨主域必填。
				//如果使用了票据登录，标记票据是给哪个服务使用。
				service : 'sso',
				//跨主域必填。应当保持和当前页面的主域一致。
				domain : 'sina.com.cn',
				//跨主域必填。通过iframe实现跨域时，首先访问的地址。
				feedBackUrl : '',
				//选填。是否重置当前域名为me.domain，等同于开启document.domain = me.domain;
				setDomain : false,
				//选填。是否执行跨域广播，如果设为true，登录成功后，类似kandian.com等跨主域站点也处于登录状态。
				crossDomain : true,
				// -------- 用于跨域登录的配置项 end --------

				// -------- 其他特殊情况配置项 start --------
				//创建新登录对象时必填。
				//当创建了新的ssoController对象实例时，应当将这个实例暴露为全局对象，并将变量名作为name参数。
				name : 'sinaSSOController'
			}, loginParam);

			//未在上面和 loginParam 中列举的属性将被忽略
			loginOptions = parseParam(loginOptions, spec);

			//如果创建了新的ssoController对象实例，则必须按照所给的name暴露这个对象。
			window[loginOptions.name] = window[loginOptions.name] || me;

			//将未在 loginParam 中列举的属性混合到ssoController对象实例
			...
			//hashExtra列举的属性，如果loginOptions里面存在对应的属性值，就混合到 loginExtraQuery 中
	...
	(function() {
		this.checklogin = function(obj) {
			var $empty = function(){};
			obj.onComplete = obj.onComplete || $empty;
			obj.onSuccess = obj.onSuccess || $empty;
			obj.onFailure = obj.onFailure || $empty;

			var userinfo;
			if (userinfo = me.getSinaCookie()) {
				obj.onComplete(userinfo);
				obj.onSuccess(userinfo);
				return true;
			}
			if (!obj["proxypage"]) obj["proxypage"] = "//" + me.domain + "/sso/proxy.html";
			var domain = location.href.match(/^[^\/]*:\/\/[^\/]+\//)[0]; // for postMessage
			var arrData = {
				action:"js",
				entry:me["entry"],
				proxypage:obj["proxypage"],
				domain:domain
			};
			wnRequest.send("http://i.sso.sina.com.cn/html/loginproxy.html#"+httpBuildQuery(arrData), function(data) {
				var onComplete = function(result) {
					obj.onComplete(result);
					if (result) {
						obj.onSuccess(result);
					} else {
						obj.onFailure(result);
					}
				};
				console.log('222222222');
				//console.log(data);
				if (data.result) {
					me.autoLogin(onComplete, true);
				} else {
					onComplete(null);
				}
			});
		...
		wnRequest.send = function( url, callback ) {  
			...
			if(window.postMessage) { // 优先选用postMessage的方式
				_debug("use postMessage");
				var _type = "message";
				var _cb = function (e) {
					_debug(e);
					if (e.origin != "http://i.sso.sina.com.cn" || !e.data.match(/^sso:/)) return;
					callback(parse_str(e.data.replace(/^sso:/,"")));
					var type = "message";
					if (_isIE) {  
						window.detachEvent('on' + type, _cb);  
					} else {  
						window.removeEventListener(type, _cb, false);  
					}  
				};
	...
	this.login = function(username, password, savestate) {
		...
		ssoLoginTimer.start(me.loginTimeout, function(){
			...
			me.callbackLoginStatus({"result":false, "errno":-1, "reason":unescape("%u767B%u5F55%u8D85%u65F6%uFF0C%u8BF7%u91CD%u8BD5")});
		});
	...
	this.ssoLogoutCallBack = function (result) {
		...
		me.crossDomainAction('logout',function(){callbackLogoutStatus({'result':true});});
	};
	...
	this.checkAltLoginName = function(username, callback){
		...
		url = makeURL(url, {'name': username, 'type':'json', 'callback':me.name + '.checkAltLoginNameCallback'});
	...
	this.checkLoginState = function(callback) {
		if(getType(arguments[0]) === 'object'){
			return loginproxy.checklogin(arguments[0]);
		}
		if (callback) {
	...
	var sinaAutoLogin = function (callback) {
		...
		if (me.domain == 'sina.com.cn') { // 如果是新浪域的自动登录，则自动登录微博
	...
	var getType = function(item){
		...
		return Object.prototype.toString.call(item).
			replace(/^\[object\s|\]$/gi, '').toLowerCase();
	};
	//判断是否为 safari 浏览器
	...
	me.name = "_sso_" + makeNonce(8);
	eval("window."+me.name + " = me;");
};
/*   
...
(function(){
 ...
 this.Cookie = {
	decode: function (input) {  
		...
		i = 0;
		while (i < input.length) {  
			enc1 = keys[parseInt(input.substr(i,2), 16)]; i+=2; 
			enc2 = keys[parseInt(input.substr(i,2), 16)]; i+=2;  
			enc3 = keys[parseInt(input.substr(i,2), 16)]; i+=2; 
			enc4 = keys[parseInt(input.substr(i,2), 16)]; i+=2;  
			chr1 = (enc1 << 2) | (enc2 >> 4);  
			chr2 = ((enc2 & 15) << 4) | (enc3 >> 2);  
			chr3 = ((enc3 & 3) << 6) | enc4;  
			output = output + String.fromCharCode(chr1);  
			if (enc3 != undefined && enc3 != 64) {  
				output = output + String.fromCharCode(chr2);  
			}  
			if (enc4 != undefined && enc4 != 64) {  
